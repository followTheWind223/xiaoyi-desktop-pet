...
> 当前版本：`v0.3.0`。这是可安装、可卸载、可升级保留配置的早期版本；语音运行时和 Agent 工具仍在开发中。

## 下载与安装

前往 [Releases](https://github.com/followTheWind223/xiaoyi-desktop-pet/releases/latest) 下载：

- `Xiaoyi-Desktop-Pet-Setup-0.3.0-x64.exe`：推荐普通用户使用。安装向导支持选择当前用户或所有用户、自定义安装路径、创建桌面与开始菜单快捷方式。
- `Xiaoyi-Desktop-Pet-Portable-0.3.0-x64.exe`：免安装便携版，适合临时体验和开发验证。
...
- 单击打开桌宠下方快捷输入，模型等待与输出期间自动联动角色动作。
...
- 右键桌宠：打开原生菜单，可进入完整对话、调整回复气泡停留时间、选择动作、切换角色或修改窗口行为。
