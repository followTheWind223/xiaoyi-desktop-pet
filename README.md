...
> 当前版本：`v0.2.2`。这是可安装、可卸载、可升级保留配置的早期版本；语音运行时和 Agent 工具仍在开发中。

## 下载与安装

前往 [Releases](https://github.com/followTheWind223/xiaoyi-desktop-pet/releases/latest) 下载：

- `Xiaoyi-Desktop-Pet-Setup-0.2.2-x64.exe`：推荐普通用户使用。安装向导支持选择当前用户或所有用户、自定义安装路径、创建桌面与开始菜单快捷方式。
- `Xiaoyi-Desktop-Pet-Portable-0.2.2-x64.exe`：免安装便携版，适合临时体验和开发验证。
...
| AI 对话气泡 | 模型配置 |
| --- | --- |
| ![AI 对话气泡](artifacts/bubble-window-preview.png) | ![模型配置](artifacts/02-model-settings.png) |
...
- 右键桌宠：打开原生菜单，可切换角色、手动左右走、锁定位置、启用点击穿透或打开设置。
- `Ctrl + Alt + Enter`：打开文字输入气泡。
...
项目在桌宠产品交互和 Hatch-Pet 兼容思路上参考了 [git2968/Desktop-pet](https://github.com/git2968/Desktop-pet)，本仓库使用 Electron、Vue 3 和独立的窗口/IPC 架构重新实现。

源码公开不代表仓库中所有角色形象、图片、声音和其他素材可以自由商用。内置 Rem 角色包用于格式适配和功能演示，其角色形象相关权利归原权利方所有。重新分发或用于商业用途前，请自行确认相应素材授权；欢迎使用拥有明确授权的原创角色包替换。
